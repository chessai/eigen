...
template <class T>
Map< FFT<T> > fft(void *p, int r, int c) {
    return Map< FFT<T> >((T*)p, r, c);
}
...
extern "C" RET eigen_fwd(int code_dst, int code_src, void* p, int r, int c)
{
    GUARD_START
    switch (code_dst) {
        case 2: break;
...
extern "C" RET eigen_inv(int code_dst, int code_src, void* p, int r, int c)
{
    GUARD_START
    switch (code_dst) {
      case 2: break;
